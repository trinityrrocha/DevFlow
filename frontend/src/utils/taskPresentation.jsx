import {
  Bug, Code2, FileCode2, Gauge, GitPullRequestArrow, Layers3,
...
export const TASK_TYPE_PRESENTATION = Object.freeze({
  ...
  BUG_REPORT: { label: 'Report Bug', icon: Bug }
});
...
export function TaskTypeIcon({ code, label }) {
  const presentation = TASK_TYPE_PRESENTATION[code]
    || { label: label || 'Tipo de tarefa', icon: getTaskCategory(code) === 'BUG' ? Bug : Code2 };
