...
export default function Tasks() {
  ...
  const [filters, setFilters] = useState({ search: '', state: '', category: '', priority: '', overdue: '' });
  ...
  return <div className="animate-fadeIn space-y-6">
    ...
    <section className="card p-4"><div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6"><label className="relative self-end"><span className="sr-only">Buscar tarefas</span><Search className="absolute left-3 top-2.5 h-5 w-5 text-slate-400" /><input value={filters.search} onChange={(event) => updateFilter('search', event.target.value)} placeholder="Buscar titulo ou codigo" className="field pl-10" /></label><Filter value={filters.state} onChange={(value) => updateFilter('state', value)} options={stateOptions} placeholder="Todos os estados" /><Filter value={filters.category} onChange={(value) => updateFilter('category', value)} options={[["BUG", "Bug"], ["DEV", "Dev"]]} placeholder="Todas as categorias" /><Filter value={filters.priority} onChange={(value) => updateFilter('priority', value)} options={priorities.map((item) => [item.id, priorityDisplayName(item)])} placeholder="Todas as prioridades" /><Filter value={filters.overdue} onChange={(value) => updateFilter('overdue', value)} options={[["true", "Atrasadas"], ["false", "Dentro do prazo"]]} placeholder="Todos os prazos" /><label className="text-xs font-semibold text-slate-600 dark:text-slate-300"><span className="mb-1 block">Agrupar por</span><select aria-label="Agrupar por" value={groupBy || ''} disabled={groupBy === null} onChange={(event) => changeGrouping(event.target.value)} className="field disabled:cursor-wait disabled:opacity-60">{TASK_GROUP_OPTIONS.map(([value, text]) => <option key={value || 'none'} value={value}>{text}</option>)}</select></label></div>{preferenceError && <p role="alert" className="mt-3 text-xs text-red-700 dark:text-red-300">Não foi possível salvar sua preferência: {preferenceError}</p>}</section>
