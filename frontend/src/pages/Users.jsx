...
import { KeyRound, Loader2, Pencil, Plus, Shield, ShieldOff, UserRound } from 'lucide-react';
...
export default function Users() {
  ...
  const openEdit = async (user) => {
    setTemporaryPassword(''); setError('');
    try { const response = await api.get(`/users/${user.id}`); setEditing({ ...response.data.user, phone: response.data.user.phone || '' }); setHistory(response.data.history); }
    catch (requestError) { setError(errorMessage(requestError)); }
  };
