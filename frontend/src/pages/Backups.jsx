...
import { Archive, CheckCircle2, Loader2, Plus, RefreshCw, RotateCcw, Trash2 } from 'lucide-react';
...
export default function Backups() {
  ...
  const label = { 'create-backup': 'Criando backup...', 'verify-backup': 'Verificando...', 'restore-backup': 'Restaurando...', 'delete-backup': 'Excluindo...' }[operation?.operation];
  ...
  return <div className="space-y-6">
    ...
    {busy && <div className="flex items-center gap-2 rounded-lg border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-700 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-300"><Loader2 className="h-4 w-4 animate-spin" />{label || operation.message}</div>}
    <section className="card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 text-xs uppercase text-slate-500 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-400"><tr><th className="px-4 py-3">Data</th><th className="px-4 py-3">Versao</th><th className="px-4 py-3">Migration</th><th className="px-4 py-3">Tamanho</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Acoes</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-800">{catalog?.backups?.map((backup) => <tr key={backup.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50"><td className="px-4 py-4"><div className="flex items-center gap-2"><Archive className="h-4 w-4 text-indigo-500" />{date(backup.createdAt)}</div><p className="mt-1 max-w-52 truncate text-xs text-slate-400" title={backup.filename}>{backup.filename}</p></td><td className="px-4 py-4">{backup.applicationVersion || '—'}</td><td className="px-4 py-4">{backup.databaseMigration || '—'}</td><td className="px-4 py-4">{size(backup.sizeBytes)}</td><td className="px-4 py-4"><span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium ${backup.status === 'verified' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}`}>{backup.status === 'verified' && <CheckCircle2 className="h-3 w-3" />}{backup.status === 'verified' ? 'Verificado' : 'Disponivel'}</span></td><td className="px-4 py-4"><div className="flex justify-end gap-2"><button type="button" className="btn-secondary px-3" disabled={busy} onClick={() => queue('post', `/operations/backups/${backup.id}/verify`)}><RefreshCw className="mr-1 h-4 w-4" />Verificar</button><button type="button" className="btn-secondary px-3" disabled={busy} onClick={() => setConfirmation({ type: 'restore', backup })}><RotateCcw className="mr-1 h-4 w-4" />Restaurar</button><button type="button" className="rounded-md p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40" aria-label={`Excluir ${backup.filename}`} disabled={busy} onClick={() => setConfirmation({ type: 'delete', backup })}><Trash2 className="h-4 w-4" /></button></div></td></tr>)}</tbody></table>{catalog && catalog.backups.length === 0 && <p className="p-8 text-center text-sm text-slate-500">Nenhum backup encontrado.</p>}{!catalog && <p className="p-8 text-center text-sm text-slate-500">Carregando backups...</p>}</div></section>
