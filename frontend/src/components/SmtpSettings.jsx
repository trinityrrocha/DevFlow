...
export default function SmtpSettings() {
  ...
  const save = async (event) => {
    ...
    try {
      ...
      setSettings({ ...initial, ...data, password: '' }); setDirty(false);
  ...
  return <section className="card p-5"><h2 className="mb-5 flex items-center gap-2 text-lg font-semibold"><Mail className="h-5 w-5 text-indigo-600" />Servidor SMTP</h2>
    <form onSubmit={save} className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-medium">Seguranca<select className="field mt-1" value={settings.security} onChange={(event) => updateSecurity(event.target.value)}><option value="starttls">STARTTLS</option><option value="ssl_tls">SSL/TLS direto</option></select></label>
        ...
        <label className="text-sm font-medium">Senha SMTP<input type="password" value={settings.password} onChange={(event) => field('password', event.target.value)} maxLength={4096} autoComplete="new-password" placeholder="Preencha apenas para alterar" className="field mt-1" /><span className="mt-1 block text-xs font-normal text-slate-500">{settings.has_password ? 'Uma senha esta salva e nunca sera exibida.' : 'Nenhuma senha salva.'}</span></label>
      ...
      <div className="flex flex-wrap items-end gap-3 border-t pt-4"><Field className="min-w-64 flex-1" label="Destinatario do teste" type="email" value={recipient} onChange={setRecipient} placeholder="Vazio usa o Super Admin" /><button type="button" onClick={test} disabled={dirty || busy || !canTest} className="btn-secondary"><Send className="mr-2 h-4 w-4" />{busy === 'test' ? 'Testando...' : 'Testar conexao'}</button><button disabled={Boolean(busy)} className="btn-primary">{busy === 'save' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Salvar</button></div>
