...
Somente `devflow-nginx` publica portas. PostgreSQL usa `devflow_internal`, sem porta do host, e o volume nomeado `devflow_postgres_data` aponta para `/opt/devflow/storage/postgres`. Backend e updater compartilham apenas a fila nomeada `devflow_updater_requests`.
