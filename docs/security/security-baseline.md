...
O download e uma leitura direta, nao entra na fila operacional e exige sessao de Super Admin. O backend recebe `/opt/devflow/backups` como bind mount somente leitura, resolve o ID exclusivamente pelo catalogo sanitizado, compara raiz e arquivo canonicos, recusa symlink/tamanho divergente e transmite o arquivo sem buffer integral. Os arquivos cifrados usam `root:devflow-ops 0640` e o diretorio `0750`; nenhum caminho local e retornado a interface.
