...
> Em `0.4.13-alpha`, reconcilie imagens e migre o estado para o schema v2 antes de executar o dry-run da publicação. A detecção de `fullpassword_nginx` não autoriza integração nem impede o estágio interno.
...
- `/api/health` informa a versão instalada, commit quando suportado e migration `001_initial_schema.sql`;
