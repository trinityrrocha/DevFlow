...
> Em `0.4.6-alpha`, instale primeiro em loopback quando 80/443 estiverem ocupadas. A detecção de `fullpassword_nginx` não autoriza integração nem impede o estágio interno.
...
- `/api/health` informa `0.4.6-alpha` e migration `001_initial_schema.sql`;
