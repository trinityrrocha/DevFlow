...
try {
  ...
  check('main with current version', current.status === 0 && current.stdout.trim() === '0.6.22-alpha');

  const patchFixture = resolve(temporary, 'patch');
  writeFixture(patchFixture, '0.6.22-alpha');
  check('main after patch increment', validateDirectory(patchFixture).status === 0);

  const minorFixture = resolve(temporary, 'minor');
  writeFixture(minorFixture, '0.6.22-alpha');
